...
from linear_head_no_lora.get_data_loaders import get_data_loader

from linear_head_no_lora.train import train_model
from linear_head_no_lora.validate import validate_model
from linear_head_no_lora.test import test_model, test_model_with_samples

from linear_head_no_lora.train_utils.train_metrics_utils import (
    visualize_val_results,
    visualize_val_confusion_matrix,
    save_val_run_metadata,
)
from linear_head_no_lora.test_utils.test_metrics_utils import (
    visualize_test_confusion_matrix,
    save_run_test_metadata,
)
from linear_head_no_lora.test_utils.test_metrics_utils_for_samples import (
...
def get_model(
    ...
    # 1-layer 선형 분류기 (Linear Probing의 정석)
    model.head = nn.Sequential(nn.Linear(embed_dim, num_classes))

    # Linear Probing 모드: 백본 전체 동결
...
def get_modules(
    ...
    # loss function (Cross Entropy Loss for baseline)
...
def run_train_val_process(
    ...
    # 결과물 저장 경로
    results_dir = Path(f"results_no_lora/{model_name}/{dataset_name}/val")
...
def run_testset_process(
    ...
    # 결과물 저장 경로
    results_dir = Path(f"results_no_lora/{model_name}/{dataset_name}/test")
    results_dir.mkdir(parents=True, exist_ok=True)
...
def run_sample_test_process(
    ...
    # 결과물 저장 경로
    results_dir = Path(f"results_no_lora/{model_name}/{dataset_name}/test/with_samples")
    results_dir.mkdir(parents=True, exist_ok=True)
