...
# 출력 폴더 그룹 디렉토리
BASE_SPLIT_DIR = CLS_DATA_DIR / "base_split_folders_25pct"
TRAIN_FOLDERS_DIR = BASE_SPLIT_DIR / "train"
VAL_FOLDERS_DIR = BASE_SPLIT_DIR / "val"
TEST_FOLDERS_DIR = BASE_SPLIT_DIR / "test"
...
def greedy_split_folders(category_folders: list[tuple[Path, int]]) -> dict[str, list[Path]]:
    ...
    assigned_folders = {"train": [], "val": [], "test": []}
    ...
    for folder_path, count in category_folders:
        ...
        for bin_name in ["train", "val", "test"]:
...
def main():
    ...
    TEST_FOLDERS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for category, folders in category_groups.items():
        ...
        print(f"Train: {counts['train']:4d}장, Val: {counts['val']:4d}장, Test: {counts['test']:4d}장")
    ...
    print(f"Test  : {global_counts['test']:5d}장")
