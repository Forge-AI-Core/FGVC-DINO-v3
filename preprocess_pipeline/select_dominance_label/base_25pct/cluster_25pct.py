...
# 크롭 데이터 디렉토리
CROPPED_DATA_DIR = CLS_DATA_DIR / "crop_images_25pct"
# 크롭 데이터 어노테이션 경로
CROPPED_ANNOTATION_PATH = CLS_DATA_DIR / "corrected_crop_annotations_25pct.json"


# 임베딩 파일 정보 보관소 디렉토리
EMBEDDING_FILE_DIR = CLS_DATA_DIR / "embeddings"
# 임베딩 파일
EMBEDDING_FILE_PATH = EMBEDDING_FILE_DIR / "embeddings_25pct.npy"
# 이미지 경로 파일
IMAGEPATH_FILE_PATH = EMBEDDING_FILE_DIR / "image_paths_25pct.txt"


# 클러스터링 파일 보관소 디렉토리
CLUSTERED_GROUPS_DIR = CLS_DATA_DIR / "base_clustered_groups_25pct"
...
def get_dataloader(batch_size: int) -> tuple[DataLoader, list[Path]]:
    ...
    image_paths = sorted(list(CROPPED_DATA_DIR.glob(pattern="*.jpg")))
...
def apply_clustering(final_embeddings: torch.Tensor, image_paths: list[Path]) -> None:
    ...
    clustering = AgglomerativeClustering(
        n_clusters=None, metric="cosine", linkage="average", distance_threshold=0.18
    )
    ...
    for path, label in zip(image_paths, labels):
        target_folder = CLUSTERED_GROUPS_DIR / (
...
def main(hyperparam_path: Path = Path("hyperparams.yaml")) -> None:
    """임베딩 추출 및 1차 클러스터링을 순차적으로 실행합니다."""
