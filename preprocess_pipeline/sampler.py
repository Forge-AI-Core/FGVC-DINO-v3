...
def sample_representative_images() -> None:
    if SAMPLED_OUTPUT_DIR.exists():
        shutil.rmtree(SAMPLED_OUTPUT_DIR)
    SAMPLED_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    cluster_folders: list[Path] = sorted(
        [d for d in CLUSTERED_GROUPS_DIR.iterdir() if d.is_dir()]
    )
    ...
    for folder in cluster_folders:
        ...
        destination_path = SAMPLED_OUTPUT_DIR / largest_img.name
    ...
    print(f"출력 경로: {SAMPLED_OUTPUT_DIR}")
